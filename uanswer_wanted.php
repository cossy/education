...
<div id="input_title" style="font-family: '宋体';font-size:15;float:right;margin-right='30px';width:700px;margin-top:10px">
	...
	<div id="input_title" style="position:relative; float:left;width:100%">
		...
		$mygrade="";
		$_sql="select grade from student where ID=$ID";
		$result=mysql_query($_sql);  
		if($row=mysql_fetch_array($result)){
			$mygrade=$row['grade'];
		}
		$_sql="select w.ID,w.subject,w.title,w.number,w.agreeID,w.done,w.time,w.addition,s.nickname from wanted w left join student s on w.personID=s.ID where w.grade='$mygrade'";
		...
		while($row=mysql_fetch_array($result)){
			...
			echo "<div class='row'><div class='col-md-6'><Strong>$i.$title</Strong></div><div class='col-md-6'><span style='margin-right:30px'><font color='blanchediamond'>$subject</font></span><span style='margin-right:30px'><font color='blanchediamond'>$nickname</font></span><span><font color='blanchediamond'>$time</font></span></div></div>";
			echo "<div>$addition</div>";
			echo "<div id='startReply".$wantedID."'>";
			$_sql1="select s.nickname,a.answer,a.time from answer a left join student s on a.answerID=s.ID where a.questionID=$wantedID";
			//echo $_sql1;
			...
			echo "</div>";
			echo "<div id='answer".$wantedID."' style='border:1px solid gray;width:500px;min-height:100px;height:auto;display:none'><div contentEditable='false'><a class='file'>添加音频<input type='hidden' name='MAX_FILE_SIZE' value='12621114' /><input type='file' onchange='addAudio(".$wantedID.")' name='addaud".$wantedID."' id='addaud".$wantedID."'></a><a class='file'>添加图片<input type='file' onchange='addPicture(".$wantedID.")' name='addpic".$wantedID."' id='addpic".$wantedID."'></a></div><div id='content".$wantedID."' style='width:497px;min-height:80px;height:auto;' contentEditable='true'></div></div>";
			echo "<div><button id='change".$wantedID."' onclick='mysubmit(".$wantedID.",this)'>回答</button></div>";
		}
		?>
  </div>
</div>
<SCRIPT src="js/update8.js" type=text/javascript></SCRIPT>
	<script type='text/javascript'>
		function mysubmit(id,button){
			var div='answer'+id;
			document.getElementById(div).style.display='block';
			button.innerHTML="收起";
			var btn=document.createElement( "button");
			btn.setAttribute("id","send"+id);
			btn.setAttribute( "onclick", "send("+id+")");
			btn.innerHTML="回答";
			button.parentElement.appendChild(btn);
			button.setAttribute( "onclick", "back("+div+","+button.id+","+btn.id+","+id+")");
		}
		...
		function send(id){
            ...
            formData.append("answer", content.innerHTML);
            formData.append("picture", document.getElementById("addpic"+id).files[0]);
            formData.append("audio", document.getElementById("addaud"+id).files[0]);  
            var oReq = new XMLHttpRequest();
			oReq.open( "POST", "cgi/answer_wanted.php" , true );
			oReq.onload = function(oEvent) {
				  if (oReq.status == 200) {
					   ...
					   font1.setAttribute("color","blue");
					   font1.innerHTML=data.name+":"+data.content;
					   div1.appendChild(font1);
					   var div2=document.createElement("div");
					   div2.setAttribute("class","col-md-4");
					   var font2=document.createElement("font");
					   font2.setAttribute("color","blanchediamond");
					   font2.innerHTML=data.time;
					   div2.appendChild(font2);
					   div.appendChild(div1);
					   div.appendChild(div2);
					   document.getElementById("startReply"+id).appendChild(div);
		...
		function back(div,button,btn,id){
			div.style.display='none';
			button.innerHTML="回答";
			button.parentElement.removeChild(btn);
			button.setAttribute( "onclick", "mysubmit("+id+","+button.id+")");
		}
		function addAudio(id){
			var docObj=document.getElementById("addaud"+id);
			if(docObj.files &&docObj.files[0]){
				//火狐下，直接设img属性
				var audio=document.createElement("audio");
				audio.setAttribute( "controls","controls");
				var source=document.createElement("source");
				source.setAttribute("src",window.URL.createObjectURL(docObj.files[0]));
				audio.appendChild(source);
				document.getElementById("content"+id).appendChild(audio);
			}
		}
		function addPicture(id){
			var docObj=document.getElementById("addpic"+id);
			if(docObj.files &&docObj.files[0]){
				//火狐下，直接设img属性
				var img=document.createElement( "img");
				img.setAttribute( "src", window.URL.createObjectURL(docObj.files[0]));
				img.setAttribute( "style","width:450px;height:150px");
				document.getElementById("content"+id).appendChild(img);
			}
		}
	</script>
<SCRIPT type="text/javascript">
