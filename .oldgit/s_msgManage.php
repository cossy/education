...
<div id="input_title" style="font-family: '宋体';font-size:15;float:right;margin-right='30px';width:700px;margin-top:10px">
	...
	<div id="input_title" style="position:relative; float:left;width:100%">
		...
		session_start();
		$_sql="select s.nickname,m.time,m.event,m.ID from message m left join student s on m.src=s.ID where m.done='F' and m.srctype='student' and m.dst=$ID and m.dsttype='student'";
		...
		session_start();
		$_sql="select s.nickname,m.time,m.event,m.ID from message m left join student s on m.src=s.ID where m.done='T' and m.srctype='student' and m.dst=$ID and m.dsttype='student'";
		mysql_select_db("pengbo",$link);
		mysql_query("set names utf8");//编码设置为utf8
		$result=mysql_query($_sql);  
		while($row=mysql_fetch_array($result)){
		...
		$_sql="select t.nickname,m.time,m.event,m.ID from message m left join teacher t on m.src=t.ID where m.done='T' and m.srctype='teacher' and m.dst=$ID and m.dsttype='student'";
		$result=mysql_query($_sql);  
		while($row=mysql_fetch_array($result)){
			$j++;
			$name=$row['nickname'];
